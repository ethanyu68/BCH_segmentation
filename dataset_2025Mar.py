...
class Dataset(data.Dataset):
    def __init__(self, path_fdr, opt, fold = None, phase = 'train', normalize = True, patch=False):
        super(Dataset, self).__init__()
        df_fold = pd.read_csv(path_fdr + '/patients_list_5folds.csv')
        # test images
        ls_patients = df_fold['fold' + str(fold)]
        self.ls_test_images = []
        ...
        for f in range(1,6):
            ...
            for patient in ls_patients:
                path_patient = os.path.join(path_fdr, patient)
                self.ls_images += [path_patient +'/' + x for x in os.listdir(path_patient) if 'seg.' not in x and 'dmap.' not in x]
        self.ls_train_images = self.ls_images[:int(0.7 * len(self.ls_images))]
        self.ls_val_images = self.ls_images[int(0.7 * len(self.ls_images)):]
        if phase == 'train':
            self.num = len(self.ls_train_images)
        elif phase == 'val':
            self.num = len(self.ls_val_images)
        else:
            self.num = len(self.ls_test_images)
        self.phase = phase
        self.args = opt
        self.patch = patch
    ...
    def __getitem__(self, item):
        if self.phase == 'train':
            path_image = self.ls_train_images[item]
        elif self.phase == 'val':
            path_image = self.ls_val_images[item]
        else:
            path_image = self.ls_test_images[item]
        path_dmap = path_image[:-4] + '_dmap.npy'
        path_label = path_image[:-4] + '_seg.npy'
        ...
        return torch.from_numpy(img.copy()).float(), \
               torch.from_numpy(label.copy()), path_image.split('.')[-2]
